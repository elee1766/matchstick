## Lua 5.5 FFI bindings.
## Compiles Lua from vendored C sources directly into the binary.

import std/os

const luaSrcDir = currentSourcePath.parentDir.parentDir.parentDir / "vendor" / "lua55" / "src"

template luaSrc(file: string): string = luaSrcDir / file

# Compile all Lua 5.5 C sources (excluding lua.c and luac.c standalone binaries)
...
# Pseudo-indices — mirrors C macro: -(INT_MAX/2 + 1000) from lua.h
const LUA_REGISTRYINDEX* = cint(-(high(cint) div 2 + 1000))
...
proc luaL_makeseed*(L: LuaState): cuint {.importc, cdecl.}
proc lua_newstate*(f: LuaAlloc, ud: pointer, seed: cuint): LuaState {.importc, cdecl.}
...
proc luaLimitedAlloc(ud: pointer, p: pointer, osize: csize_t, nsize: csize_t): pointer {.cdecl.} =
  ...
  ## Uses libc malloc/realloc/free directly (not Nim's GC-aware allocator)
  ## to avoid issues with Lua's longjmp-based error handling.
  ...
  if nsize == 0:
    ...
    let newp = if p == nil: c_malloc(nsize)
               else: c_realloc(p, nsize)
    if newp != nil:
...
proc luaL_newstate_limited*(allocState: ptr LuaAllocState, memLimit: int = defaultLuaMemoryLimit): LuaState =
  ...
  ## We use a temporary default state to generate a proper seed via luaL_makeseed,
  ## then create the real memory-limited state with that seed.
  allocState.allocated = 0
  allocState.limit = memLimit
  let tmpL = luaL_newstate()
  let seed = if tmpL != nil: luaL_makeseed(tmpL) else: 0.cuint
  if tmpL != nil: lua_close(tmpL)
  result = lua_newstate(luaLimitedAlloc, cast[pointer](allocState), seed)
