...
proc nftValidate*(ruleset: string): NftResult =
  ## Validate a JSON ruleset via `nft -j -c -f <tmpfile>` (dry-run).
  nftRunWithFile(@["-j", "-c", "-f"], ruleset)

proc nftApply*(ruleset: string): NftResult =
  ## Apply a JSON ruleset via `nft -j -f <tmpfile>`. Requires root.
  ## JSON is the canonical nftables machine format — avoids text
  ## escaping/quoting edge cases.
  nftRunWithFile(@["-j", "-f"], ruleset)
