...
##   nim c --os:emscripten --cpu:wasm32 -d:noSystem -d:release \
##     --passL:"-s EXPORTED_FUNCTIONS=['_loadAndRender'] -s EXPORTED_RUNTIME_METHODS=['ccall','cwrap']" \
##     -o:web/static/playground.js web/wasm/playground.nim

import std/[json, options, tables, sequtils]
...
proc loadAndRender(luaCode: cstring, format: cstring): cstring {.exportc, cdecl.} =
  ## Called from JS via Module.ccall('loadAndRender', 'string', ['string','string'], [code, fmt])
  let code = $luaCode
  let fmt = $format
  ...
  # Use a dummy path (no filesystem in WASM)
